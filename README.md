...
### 2023-11-10 메인페이지 완성, 현재 header를 비롯한 css 및 추가 작업 시작함. 로그인 기능은 현재로썬 사용하는 방법이 보안성 이슈가 있어 보류되었고, 기타 기능들을 먼저 건드릴 예정. 현재 header 재작업을 통해 제목 클릭 시 메인 페이지, 오른쪽에 로그아웃 버튼(물론 로그인 기능이 있는건 아니라서 해당 버튼을 통해 현재 로그인한 유저를 바꾸는 기능이 될듯)과 알람 기능을 만들 예정. 현재 메인 페이지는 본래 계획이었던 2x2 그리드가 너무 보여주고 싶던 정보대비 크기가 작다는 이슈가 있어 1줄로 대체되었고, 특이한 아이디어가 나오는게 아니면 현 ui 유지할듯. 현재로썬 currentMember를 통해 로그인한 사람으로 표기하고 있는데, 해당 기능이 정상작동하도록 로그인 기능이 나오던 전역변수화를 시키던 어쩌던 해서 완성될 경우 projectPlanList나 ApprovalList도 작성자가 들어가게 수정되어야 함.

### 2023-11-13 쿠키 수정 과정에서 오류 발견. projectPlanList에 Member내 거대한 데이터가 다들어가고 있어 사람이 너무 많아지면 쿠키 크기가 기하급수적으로 커지는 현상을 발견하여 addMemberV2의 수정이 필요함. 머리자르면서 계획한 사항인 private페이지 작성, orgTree에 민감정보 가리기, 공지작성 개선, Header 누르면 메인페이지로 완성. 현재 알람띄우기 기본 기능은 작성되었고 쿠키 크기를 줄이고 가독성을 높이기 위해 Member 관련 쿠키 저장 메커니즘 수정중. 쿠키 크기가 줄어들면 notification 기능 완성 가능할듯. 현재도 기본 기능은 저장되어있음.

### 2023-11-14 알람 작업 계속함. 알람 자체는 이제 문제없이 작동하지만 알람 창이 지금 안이쁘게 나와서 현재는 dropdown으로 사용하고 있지만 select나 menu를 이용하게 바꿀까 확인중. 2줄 이상의 내용을 dropdown이 담을 수 없다면 현재로써는 다른 방법을 갈구해야함. 어제 못했던 member 저장 메커니즘 및 documentedProjectV2 재작업 완료. 결재는 이제 자동으로 참여자에서 가장 높은 계급을 찾아 해당 사람에게 결재가 넘어가게 만듬.

### 2023-11-15 dropdown은 유지하되 각 메뉴마다 독자 아이콘을 부여하고 (아이콘)from이 title을 제출/작성하였습니다 식으로 처리하였음. 개인페이지 링크 걸어두고 개인 업무를 총괄한 페이지를 만들지 고민중(왜냐하면 mainPage와 역할이 너무 겹침). 지금 생각에는 업무 관련해서 일종의 내부 메신저 시스템을 구축하는게 더 낫다는 생각이 들었음(왜냐하면 이게 있으면 이것저것 편리하게 사용될 여지가 큼, 그리고 공지만들때 사용하던거 그대로 가져와서 적절히 수정과 lzString 처리만 해주면 금방 만듬). 메세지 기본 틀 작성 완료. 서로 emit하도록 연결하고 큰 messageMain.vue 하나 만들어서 컴포넌트 연결만 하면 됨. 현재로써 저거 완성되면 메뉴를 조직(일정표, 조직도), 프로젝트(계획,진행,완료), 결재(현재는 결재만 있지만 완료된 결재들과 반려 등의 추가행동 시 어떻게 처리할지에 대해 추가작업 필요), 개인페이지(개인정보(수정기능은 고민중), 개인 메시지, 메인페이지), 관리자(구성원 변경, 공지)로 구성되는 사이트가 완성됨.

### 2023-11-20 몸상태가 괜찮아져서 다시 작업 시작함. 현재 메시지 기능 완성했고 사소한 css 정도 문제만 건드리면 진짜로 끝남. 내일 다시 작업 시작할때는 승인이 아닌 결재 처리, 결재 완료 리스트, 아이콘 선택 정도만 남아있게 됨. css는 계속해서 고민되는 사항임. 현재 메세지 보는 창이 너무 못생겼는데 기능상은 문제가 없지만 추가 수정이 필요해보임.

### 2023-11-21 결재를 승인/반려/취소 3개로 축소시킨 후 승인되지 않은 계획이나 업무 안 같은걸 보관할 페이지가 필요해짐. 현재 생각에는 계획 창을 수정하여 아직 결재가 통과되지 않은 것들을 따로 처리하도록 페이지 내부 탭을 추가하여 처리. 결재되지 않은 것들은 대기중, 결재 승인되면 이 탭에 보이지 않게 되며 결재 반려된 것들은 알람, 메시지를 남기며 이곳에 남아 차후 처리를 결정할 수 있게 됨. 이 차후 처리에서 삭제, 수정이 결정되며 삭제는 말 그대로, 수정은 재결재 요청을 하게 되게 할것. 현재 차후 처리 큰 틀은 만들어졌는데 결재와 저장 과정에서 뭔가 이상한게 생겨서 지금 코드 하나씩 확인해보는중. 아이콘은 정했고 sideMenu 식으로 translate해서 아이콘을 알람에 넣어주기만 하면 될듯. 이 버그 1개만 수정하면 될듯?

### 2023-11-24 3가지 문제만 해결하면 됨. 1. 현재 코드에서 모든 VueCookies, lzString을 localStroage로 바꿔서 데이터 크기 늘리기. 현재 결재 상황 문제에 있어서 Cookie 크기제한이 크게 의심되는 상황인 만큼, 이 과정을 반드시 수행해야 함. 2. 저 이전작업을 끝내고 protoPlan 페이지에서 결재자 잘 불러오는지 확인. 3. 그거 불러오는거 잘 되면 결재창에 잘 넘어가지는지, 중복결재 문제 해결하기.

### 2023-12-01 독감이슈로 1주일이나 몸져 누운 뒤 오늘 다시 작업. 문제의 원인은 DocumentedProjectV2에서 master를 삭제하고 있었기 때문에 발생하던 것으로 해당 코드를 제거하니 이슈가 해결되었음. 그래도 쿠키들을 모두 localStorage로 옮기는 작업을 했으며 나머지 포인트들이 정상작동 하는 것을 확인함. 일단 이걸 build해서 github에서 private page로 볼수 있게 띄워놓고 이 프로젝트를 일부 기능(일정, 공지)제외하고 프로젝트의 생성과 결재, 알람, 메시지 정도로 띄울 수 있도록 새로 또 하나 만들어 볼 거임.
